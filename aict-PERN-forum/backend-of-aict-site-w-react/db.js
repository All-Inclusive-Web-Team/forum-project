...
export async function getUserComments(id) {
  let data = await pool.query(  `SELECT comment.id, comment.author, comment.comment, TO_CHAR(comment.date, 'MM/DD/YYYY - HH:MIam') AS date, cardinality(comment_reaction.likes) AS likes, cardinality(comment_reaction.dislikes) AS dislikes FROM comment LEFT JOIN comment_reaction ON (comment.id = comment_reaction.comment_id) WHERE comment.users_id='${id}' GROUP BY comment.id, comment_reaction.likes, comment_reaction.dislikes ORDER BY comment.date DESC`)
  return data.rows
}
// LIKES AND DISLIKES
